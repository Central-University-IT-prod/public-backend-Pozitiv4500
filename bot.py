...
async def process_age(message: types.Message):
    ...
    if await check_user_exists(message.from_user.id):
        ...
        await message.reply(
            f"Доброго полудня, {name}! Пожалуйста в настройках телеграм введите ваш ник пользователя, а затем возвращайтесь к нам, чтобы продолжить регистрацию",
...
async def request_location(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply(
            "Пожалуйста, нажмите на кнопку ниже, чтобы отправить вашу текущую локацию или введите название вашего населенного пункта вручную.",
...
async def process_city_or_location(message: types.Message, state: FSMContext):
    if message.content_type == types.ContentType.LOCATION:
        ...
        if location:
            ...
            await message.reply(
                "Ваш населенный пункт не найден. Пожалуйста, попробуйте ввести название вашего населенного пункта ещё раз.")
...
async def edit_bio(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text(
        "Расскажите нам немного о себе: кем вы являетесь, чем увлекаетесь, какие у вас интересы? 📝")
...
async def process_city_or_location(message: types.Message, state: FSMContext):
    if message.content_type == types.ContentType.LOCATION:
        ...
        if location:
            ...
            # Город не найден, отправляем сообщение с кнопками "Всё верно" и "Наверное"
            await message.reply(
                "Ваше местоположение не найдено. Пожалуйста, попробуйте ввести ваше местоположение ещё раз.")
...
async def process_location_not(callback_query: CallbackQuery, state: FSMContext):
    await callback_query.message.edit_text(
        "Пожалуйста, введите ваше местоположение ещё раз.")  # Изменяем сообщение с приглашением ввести название пункта ещё раз
...
async def manage_travel(callback_query: types.CallbackQuery):
    await callback_query.message.edit_text(
        "Добро пожаловать в управление путешествиями! 🌟\n\nЗдесь вы можете создавать новые путешествия, просматривать список уже запланированных приключений, редактировать детали каждого путешествия и даже удалить те, которые уже завершились. Приятного путешествия! ✈️🚗🚀",
        reply_markup=manage_travel_menu)
...
async def create_trip(callback_query: types.CallbackQuery):
    await callback_query.message.edit_text(
        "Вы выбрали создание нового путешествия. Пожалуйста, введите название путешествия:")
...
async def delete_trip(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text(trip_message,
...
async def process_location(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        "Пожалуйста, введите время начала посещения в формате ГГГГ-ММ-ДД (например, 2024-03-20):")
...
async def process_location_not(callback_query: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(points=points)
    await callback_query.message.edit_text(
        "Пожалуйста, введите вашу точку маршрута ещё раз.")  # Изменяем сообщение с приглашением ввести название пункта ещё раз
...
async def process_start_date(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply(
            "Некорректный формат даты. Пожалуйста, введите дату в формате ГГГГ-ММ-ДД (например, 2024-03-20):")
        return
...
async def process_end_date(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.reply(
            "Некорректный формат даты. Пожалуйста, введите дату в формате ГГГГ-ММ-ДД (например, 2024-03-25):")
        return
    ...
    await message.reply(
        "Время начала и окончания путешествия успешно установлено!\nХотите добавить еще одну точку или завершить создание путешествия?",
        reply_markup=change)
...
async def edit_trip(callback_query: types.CallbackQuery):
    ...
    keyboard.add(
        ...
        types.InlineKeyboardButton("Редактировать описание путешествия",
                                   callback_data=f"edit_trip_description_{trip_id}"),
...
async def list_trips(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text(await format_trip_message(locations),
                                           reply_markup=back_to_menu_travels_keyboard, parse_mode="HTML")
...
async def choose_travel_type(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if not trip_data:
        await callback_query.message.edit_text(
            "У вас пока нет путешествий 😔 Выберите путешествие друзей или сами создайте путешествие.",
            reply_markup=keyboard_markup)
        return
...
@dp.callback_query_handler(
    lambda callback_query: callback_query.data == 'note_public' or callback_query.data == 'note_private')
...
@dp.message_handler(state=NoteCreation.EnterNote,
...
async def choose_notes_travel_type(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if not trip_data:
        await callback_query.message.edit_text(
            "Ничего не найдено 😔 Поробуйте выбрать другой тип путешествия или сами создайте заметку.",
            reply_markup=keyboard_markup)
        return
    keyboard_markup = types.InlineKeyboardMarkup(row_width=1)
...
async def travel_with_friends(callback_query: types.CallbackQuery):
    ...
    buttons = [
        ...
        types.InlineKeyboardButton("Просмотреть все путешествия, в которые вы добавлены 🔍",
                                   callback_data="view_all_trips"),
        types.InlineKeyboardButton("Назад ↩️", callback_data="show_menu")
    ]
    keyboard_markup = types.InlineKeyboardMarkup(row_width=1)
...
async def view_all_trips(callback_query: types.CallbackQuery):
    ...
    if not joined_trips_info:
        await callback_query.message.edit_text("Вы еще не добавлены ни в одно путешествие.",
                                               reply_markup=keyboard_markup)
        return
...
async def add_user_to_trip(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    for trip_id in unique_trip_ids:
        ...
        if trip:
            buttons.append(
                types.InlineKeyboardButton(trip['trip_name'], callback_data=f"add-user-to-trip_{trip['trip_id']}"))
...
async def plan_travel_route(callback_query: types.CallbackQuery):
    ...
    travel_route_button = types.InlineKeyboardButton("Прокладывание маршрута путешествия 🗺️",
                                                     callback_data="travel_route")
    start_route_button = types.InlineKeyboardButton("Построить маршрут до начальной точки путешествия 🏃‍♂️",
...
async def select_trip_type(callback_query: types.CallbackQuery):
    ...
    if not trips:
        await callback_query.message.edit_text(
            "Путешествия не найдены. Создайте их или попросите друзей пригласить вас в их путешествие. 🌍🚀",
            reply_markup=InlineKeyboardMarkup(row_width=1).add(
                InlineKeyboardButton("Назад ↩️", callback_data="travel_route")))
        return
    ...
    for trip in trips:
        keyboard_markup.add(
            InlineKeyboardButton(trip['trip_name'], callback_data=f"route_select_trip_{trip['trip_id']}"))
...
async def build_trip_route(callback_query: types.CallbackQuery):
    ...
    if len(trip_points) == 1:
        await callback_query.message.edit_text("В маршруте только одна локация. По ней нельзя построить маршрут :(",
                                               reply_markup=keyboard_markup)
    ...
    if not route_points:
        await callback_query.message.edit_text("Маршрут для выбранного путешествия невозможно построить",
                                               reply_markup=keyboard_markup)
...
@dp.callback_query_handler(
    lambda callback_query: callback_query.data in ['route_to_my_trips_start', 'route_to_friend_trips_start'])
async def select_trip_type_for_start(callback_query: types.CallbackQuery):
    ...
    for trip in trips:
        keyboard_markup.add(
            InlineKeyboardButton(trip['trip_name'], callback_data=f"start_select_trip_{trip['trip_id']}"))
...
async def start_select_trip(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        "Пожалуйста, отправьте своё текущее местоположение 📍, чтобы мы могли построить маршрут до начальной точки путешествия.")
...
@dp.callback_query_handler(lambda callback_query: callback_query.data == 'confirm_location',
                           state=Road_to_Trip.Placment)
async def confirm_location(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if not route_points:
        await callback_query.message.edit_text(
            "Карту по точкам маршрута для выбранного путешествия невозможно построить :(")
        await state.finish()
        await plan_travel_route_mess(callback_query.message)
        return
    points = [
...
async def weather_forecast_callback(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text("Выберите, какие путешествия вас интересуют для просмотра погоды:",
                                           reply_markup=travel_choice_menu)
...
async def view_weather(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    if not location_data:
        await callback_query.message.edit_text("Не удалось получить информацию о выбранной локации.",
                                               reply_markup=location_choice_menu)
        return
    ...
    if 'list' not in data:
        await callback_query.message.edit_text("Не удалось получить прогноз погоды для выбранной локации.",
                                               reply_markup=location_choice_menu)
        return
...
async def find_travel_buddies_handler(callback_query: CallbackQuery):
    ...
    if user_data:
        ...
        if travelers:
            ...
            await callback_query.message.edit_text("Подходящие путешественники не найдены.",
                                                   reply_markup=hotel_choice_menu)
    else:
        await callback_query.message.edit_text("Информация о вас не найдена. Пожалуйста, заполните профиль.",
                                               reply_markup=hotel_choice_menu)
...
async def hotel_selection_callback(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text(
        "Выберите, какие путешествия вас интересуют для просмотра достопримечатльностей:",
        reply_markup=hotel_choice_menu)
...
async def choose_location_for_hotel(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        "Выберите локацию для просмотра информации о достопримечательностях рядом с ней:",
        reply_markup=location_choice_menu)
...
async def cafe_selection_callback(callback_query: types.CallbackQuery):
    ...
    await callback_query.message.edit_text("Выберите, какие путешествия вас интересуют для просмотра кафе:",
                                           reply_markup=cafe_choice_menu)
...
async def hotel_selection_callback(callback_query: CallbackQuery):
    ...
    other_region_hotel_button = types.InlineKeyboardButton(text="Путешествия друзей 👫",
                                                           callback_data="another_region_hotel")
    ...
    await callback_query.message.edit_text("Выберите, какие путешествия вас интересуют для просмотра отелей:",
                                           reply_markup=hotel_choice_menu)
...
async def choose_hotel_callback(callback_query: CallbackQuery, state: FSMContext):
    ...
    # Отправляем сообщение с просьбой выбрать локацию
    await callback_query.message.edit_text(
        "Выберите локацию для просмотра информации о путешествии и отеле рядом с ней:",
        reply_markup=location_choice_menu)
...
secrettels_nearby(latitude: float, longitude: float, client_id = "Если сейчас у тебя уже есть ноутбук, но он не подходит под требования ниже, ничего страшного.", client_secret = "secret"):
